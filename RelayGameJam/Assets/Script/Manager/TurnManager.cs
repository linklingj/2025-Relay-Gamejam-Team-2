...
public class TurnManager : MonoBehaviour
{
    ...
    private void SpawnUnits <T>(List<T> units,bool isPlayerTeam) where T : Component
    {
        for (int i = 0; i < units.Count; i++)
        {
            //보간을 위한 비율 나누기
            //ex) 혼자일 때 0.5f(중간거리) 두명일 때 0.25f,0.75f(25%거리, 75%거리 위치) 세명 이상 부터는 꽉 채워서
            float t = units.Count == 1?0.5f:units.Count ==2?0.25f+0.5f*i: (float)i / (float)(units.Count-1);
            //보간을 통해 x위치를 구함
            float xPos = Mathf.Lerp(-spawnRange/2, spawnRange/2, t);
            //플레이어 팀 진영인지 확인
            Transform trans = isPlayerTeam ? playerSpawnTrans : enemySpawnTrans;
            //소환
            var unit = Instantiate(units[i], trans.position + new Vector3(xPos,0), Quaternion.identity);
